...
export function AdminFormCreatePlantInstance(props: Props) {
  let [formCreatPlant, setFormCreatePlant] = useState({
      plantId: {
  ...
  useEffect(() => {
    apiGetAllPlants().then(res => {
      formCreatPlant.plantId.list = res.data.plants.map(item => {
        return {
          key: item._id,
          value: item.name
        }
      });
      setFormCreatePlant(formCreatPlant)
    })
  }, [])
  return <div className="admin-forms">
    ...
    <FormGenerator title={'新建植株'} formDetail={formCreatPlant} onConfirm={values => {
